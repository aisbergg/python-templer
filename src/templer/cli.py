    ...
    def _create_context(self):
        ...
        if self.contextfile_paths:
            ...
            if len(contextfiles) == 0:
                raise IOError("No context files (*.yml, *.yaml) found in given path(s)")
    ...
    def _create_templates(self):
        ...
        if len(self.template_paths) > 1 or os.path.isdir(self.template_paths[0]):
            if os.path.exists(self.destination_path) and \
                ...
                raise ValueError("Destination exists and is not a directory. When multiple templates are specified the destination must be a directory")
        
        destination_is_dir = not os.path.exists(self.destination_path) or os.path.isdir(self.destination_path)
        
        for path in self.template_paths:
            path = os.path.abspath(path)
            if os.path.isdir(path):
                flist = self.find_files(path, file_extensions, relative_paths=True)
                for frel in flist:
                    fabs = os.path.join(path, frel)
                    dest = os.path.join(self.destination_path, os.path.splitext(frel)[0])
                    templates.append(
                        TemplateFile(
                            src=fabs,
                            dest=dest,
        ...
        if len(templates) == 0:
            raise IOError("No template files (*.j2, *.jinja2) found in given path(s)")
        ...
        for root, _, filenames in os.walk(path):
            for filename in filenames:
                if filename.endswith(tuple(extensions)):
                    if relative_paths:
                        found_files.append(os.path.relpath(os.path.join(root, filename), path))
                    else:
...
class Context(object):
    ...
    def __init__(self, variables, contextfiles):
        ...
        for ctxf in contextfiles:
            self.context = merge_dicts(self.context, ctxf.get_context(variables))
...
class ContextFile(object):
    ...
    def get_context(self, variables):
        ...
        try:
            ...
            if hasattr(e, 'problem_mark'):
                mark = e.problem_mark
                raise yaml.YAMLError(self._format_error("YAML parsing error at line {0} column {1}".format(mark.line+1, mark.column+1)))
            else:
                raise yaml.YAMLError(self._format_error("YAML parsing error: {0}".format(str(e))))
        except Exception as e:
    ...
    def _parse_defaults(self, defaults_data, env_context):
        ...
        if type(defaults_data) is not dict or len(defaults_data) == 0:
            raise TypeError(self._format_error("'defaults' must be of type 'dict' and contain at least one key"))
        ...
        for var_name, var_default in defaults_data.items():
            val = default_getter[type(var_default)](var_name, var_default, env_context)
    ...
    def _get_default_bool(self, name, default, variables):
        ...
        if name in variables:
            ...
            if b in bool_true:
                ...
                raise TypeError(self._format_error("Variable '{0}' must be of type 'bool'".format(name)))
            else:
                return variables[name]
        else:
            return default
        ...
        if name in variables:
            try:
                ...
                if self.defaults_type_check:
                    raise TypeError(self._format_error("Variable '{0}' must be of type '{1}'".format(name, type(default))))
                else:
                    return variables[name]
        else:
            return default
    ...
    def _get_default_list(self, name, default, variables):
        ...
        if name in variables:
            try:
                ...
                if self.defaults_type_check:
                    raise TypeError(self._format_error("Variable '{0}' must be a list in json format".format(name)))
                else:
                    return variables[name]
        else:
            return default
    ...
    def _get_default_dict(self, name, options, variables):
        ...
        if not 'type' in options:
            raise ValueError(self._format_error("Variable '{0}': When using a special default the 'type' is required").format(name))
        
        if options['type'] == 'choice':
            if not 'default' in options:
                raise ValueError(self._format_error("Variable '{0}': When using 'choice' the 'default' value must be specified").format(name))
            if not 'choices' in options or type(options['choices']) is not list:
                raise ValueError(self._format_error("Variable '{0}': When using 'choice' a list of 'choices' must be specified").format(name))
            ...
            if name in variables:
                ...
                if case_sensitive:
                    ...
                    import unicodedata
                    normalize_caseless = lambda s: unicodedata.normalize("NFKD", s.casefold())
                    caseless_equal = lambda s1, s2: normalize_caseless(s1) == normalize_caseless(s2)
                    for c in choices:
                ...
                if not valid_choice:
                    raise ValueError("Variable '{0}': Your choice '{1}' is not available. The available choices are: {2}".format(name, choice, ', '.join(choices)))
            ...
            if not 'delimiter' in options:
                raise ValueError(self._format_error("Variable '{0}': When using 'list' the 'delimiter' must be specified").format(name))
            if not 'default' in options or type(options['default']) is not list:
                raise ValueError(self._format_error("Variable '{0}': When using 'list' the 'default' must be specified").format(name))
            ...
            if not 'defaults' in options or type(options['defaults']) is not dict:
                raise ValueError(self._format_error("Variable '{0}': When using 'variation' the 'defaults' must be specified and be of type 'dict'").format(name))
            ...
            raise ValueError(self._format_error("Variable '{0}': Unknown special default type '{1}'. Available types are: choice, list, variation").format(name, options['type']))
...
class TemplateFile(object):
    ...
    def _write_rendered_file(self, content):
        ...
        if os.path.exists(self.dest):
            if os.path.isfile(self.dest):
                if not self.force_overwrite:
                    raise IOError(self._format_error("Destination already exists. Use '-f' flag to overwrite the file".format(self.dest)))
            else:
                raise IOError(self._format_error("Destination exists and is not a file".format(self.dest)))
        else:
            # create dir
        ...
        if self.file_mode:
            Log.debug("Setting file mode of the template file...".format(self.dest))
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        ...
        add_help=False)
    parser.add_argument('-c', '--contextfile', dest='contextfile', action='append', help="Context file to be used for rendering. Path can be either a file or directory containing multiple files (*.yml). The option can be used multiple times to specify multiple paths")
    parser.add_argument('-d', '--dynamic-contextfiles', dest='dynamic_contextfiles', default=False, action='store_true', help="Render the context files like the templates before parsing them")
    parser.add_argument('-r', '--remove-templates', dest='remove_templates', default=False, action='store_true', help="Delete the templates after rendering")
    parser.add_argument('-f', '--force', dest='force_overwrite', action='store_true', default=False, help="Overwrite existing files")
    parser.add_argument("-h", "--help", action="help", help="Show this help message and exit")
    parser.add_argument('-i', '--ignore-undefined-variables', dest='ignore_undefined_variables', action='store_true', default=False, help="Ignore undefined variables")
    parser.add_argument('-m', '--mode', dest='file_mode', help="File mode for rendered files")
    parser.add_argument('-t', '--defaults-type-check', dest='defaults_type_check', action='store_true', default=False, help="Check if the environment variables match the data type of the defaults specified in a context file")
    parser.add_argument('-v', '--verbose', dest='verbose', action='count', default=0, help="Enable verbose mode (-vv for debug mode)")
    parser.add_argument('--version', action='version', version='Templer {0}, Jinja2 {1}'.format(__version__, jinja2.__version__), help="Prints the program version and quits")
    parser.add_argument('template', nargs='+', help="File to be rendered. Path can be either a file or directory containing multiple files (*.j2)")    
    parser.add_argument('destination', help="Destination for the rendered file(s)")
